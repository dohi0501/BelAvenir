    ...
    if request.method == 'POST':
        ...
        if Form_R.is_valid():
            ...
            if Form_R.cleaned_data.get('date_debut') >= Form_R.cleaned_data.get('date_fin'):
                ...
                form_R = FormRapprt(request.POST, request.FILES)
                return render(request, 'ajout_rapport.html', {'rapport_form': form_R, 'error': erreur})
...
def Ajout_video(request, username, *args, **kwargs):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid(): 
            ...
            fichier = request.FILES.get('pdf_recapitulatif')

            if fichier:
                content_type = fichier.content_type 
                if not content_type.startswith('video/'):  # Vérifie si c'est une vidéo
                    erreur_vid = "Le fichier doit être une vidéo."
                    form_R = FormRapprt(request.POST, request.FILES)
                    return render(request, 'ajout_rapport.html', {'form': form_R, 'error': erreur_vid})
